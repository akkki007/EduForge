...
app.post("/studentSignUp", async (req, res) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      message:
        "Account request sent successfully. Please wait for the admin to accept your account. The login credentials will be sent to you via email.",
      data: {
        user: {
          id: newUser._id,
  ...
  try {
    ...
    if (!student.accepted) {
      return res.status(403).json({
        success: false,
        message:
          "Your account has not been accepted by the admin yet.Please wait for the admin to accept your account.The login credentials will be sent to you via email.",
      });
    }
  ...
  try {
    ...
    if (!teacher.accepted) {
      return res.status(400).json({
        success: false,
        message:
          "Teacher not accepted. Please wait for the admin to accept your account.Your login credentials will be sent to you via email.",
      });
    }
...
app.post("/admin/updateRequests", async (req, res) => {
  ...
  if (!id || !status || !type) {
    return res
      .status(400)
      .json({ error: "Missing required fields: id, status, or type" });
  }
  ...
  try {
    ...
    if (type === "teacher") {
      ...
      return res
        .status(200)
        .json({ message: "Teacher status updated successfully" });
    }

    return res
      .status(400)
      .json({ error: 'Invalid type. Must be "student" or "teacher"' });
  } catch (error) {
