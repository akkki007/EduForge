...
export default function Attendance() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post("http://192.168.121.47:5000/load");
  ...
  const handleSubmit = async () => {
    if (isInsert) {
      try {
        await axios.post("http://192.168.121.47:5000/insert", {
          updatedRows: rows,
          date: date.format("YYYY-MM-DD"),
          div: division,
        });
      ...
      try {
        await axios.post("http://192.168.121.47:5000/update", {
          updatedRows: rows,
          date: date.format("YYYY-MM-DD"),
          div: division,
        });
